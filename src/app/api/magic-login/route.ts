...
export async function GET(req: NextRequest) {
  ...
  console.log("[magic-login] matched:", byVpMer ? "vpMer" : byGm ? "gm" : byPresident ? "president" : byScm ? "scm" : byVpScm ? "vpScm" : byLogistics ? "logistics" : byAccounting ? "accounting" : byClaimGw ? "claimGw" : byScmNykAppr ? "scmNykApprover" : byScmNykEvp ? "scmNykEvp" : byScmNyk ? "scmNyk" : byScmNyg ? "scmNyg" : byClaimNext ? "claimNext" : "none")

  if (!byVpMer && !byGm && !byPresident && !byScm && !byVpScm && !byLogistics && !byAccounting && !byClaimGw && !byScmNykAppr && !byScmNykEvp && !byScmNyk && !byScmNyg && !byClaimNext) {
