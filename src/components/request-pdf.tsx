...
export function CombinedPdfDocument({ pages, hawbNo }: { pages: { req: any; item: any }[]; hawbNo?: string }) {
  ...
  // Single document → normal full page. Multiple documents → all sections flow continuously in
  // ONE page stream (no forced page break between documents), each still with its own header.
  if (groups.length <= 1) {
    return <Document title={title}><DocSection pages={groups[0]?.pages || pages} hawbNo={hawbNo} /></Document>
  }
  return (
    <Document title={title}>
      <Page size="A4" style={s.pageFlow} wrap>
        {groups.map((g, i) => <DocSection key={(g.req?.id || "") + i} pages={g.pages} hawbNo={hawbNo} flow />)}
        <View style={s.footer} fixed>
          <Text>Generated by Air Request System</Text>
          <Text>{companyFor(groups[0]?.req?.bu).name}</Text>
        </View>
      </Page>
    </Document>
  )
...
function DocSection({ pages, hawbNo, flow }: { pages: { req: any; item: any }[]; hawbNo?: string; flow?: boolean }) {
  ...
  // In a multi-document combined PDF each section flows continuously (a plain View, header not
  // fixed). A single document keeps the full page treatment (fixed table header + footer).
  if (flow) return <View style={{ marginTop: 8, marginBottom: 8 }} wrap>{content}</View>
  return (
    <Page size="A4" style={s.pageFlow} wrap>
      {content}
      <View style={s.footer} fixed>
