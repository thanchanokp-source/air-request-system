...
function ItemPage({ req, item, flow }: { req: any; item: any; flow?: boolean }) {
  ...
  const sig = (
    <View style={flow ? s.sigWrapFlow : s.sigWrap}>
      {signers.map((sg, i) => (
  ...
  // Flow mode (Combined / All-SO): a self-contained block that stays together
  // (wrap=false) but runs continuously — multiple SOs per page, no page break each.
  if (flow) {
    return <View style={s.soBlockFlow} wrap={false}>{body}{sig}</View>
  }
  ...
  return (
    <Document title="Booking File — Combined">
      <Page size="A4" style={s.pageFlow} wrap>
        {pages.map((p, i) => <ItemPage key={i} req={p.req} item={p.item} flow />)}
        <View style={s.footer} fixed>
          <Text>Generated by Air Request System</Text>
          <Text>{COMPANY.name}</Text>
        </View>
